...
const LinkButton: React.FC<LinkButtonProps> = ({  href, target ,children}) => {
    return <Link className="h-12 hover:bg-primary/90 font-medium  text-white px-8 inline-flex items-center rounded-full justify-center whitespace-nowrap bg-primary text-md  ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50" href={href} passHref target={target}>
        {children}
    </Link>;
}
